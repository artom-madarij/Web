...
export const mockAuthAPI = {
  login: async (email, password) => {
    ...
    if (localPart.length < 5) {
      throw new Error('Email має містити щонайменше 5 символів перед знаком @');
    }
    
    if (!ALLOWED_DOMAINS.includes(domain)) {
      throw new Error(`Дозволені домени: ${ALLOWED_DOMAINS.join(', ')}`);
    }
  ...
  register: async (userData) => {
    ...
    if (!userData.email.includes('@')) {
      throw new Error('Email має містити знак @');
    }
